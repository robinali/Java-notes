class Solution {
    public List<List<String>> solveNQueens(int n) {
        // Time: O(nˆn) Uncertain Space: O(n)
        List<List<String>> res = new ArrayList<>();
        if(n <= 0) return res;
        backTracking(res, new int[n], 0);
        return res;
    }
    
    private void backTracking(List<List<String>> res, int[] queens, int pos) {
