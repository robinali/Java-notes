class Solution {
    ...
    public double[] medianSlidingWindow(int[] nums, int k) {        
        ...
        Supplier<Double> median = (k%2 == 1) ?
            () -> (double) nums[right.first()] :
            () -> (double) (nums[left.first()] / 2.0 + nums[right.first()] / 2.0);
    
        // balance left size and reight size (if not equal, then right will be larger by one)
        Runnable balance = () ->
